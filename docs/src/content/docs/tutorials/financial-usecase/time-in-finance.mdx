---
title: Time in Insurance - Underwriter Workflow
---
...
IT systems of all kinds are plagued by many time-related complexities, from misconfigured Network Time Protocol and unexpected certificate expirations, to changing timezones and software concurrency bugs. However, systems in the insurance domain, such Policy Administration Systems, are additionally accutely affected by the following common requirements and challenges...
...
Traditional off-the-shelf Policy Administration Systems are constrained by how and when data can be updated. Insurance companies invariably need to augment these systems with workarounds to present the right data for users.

Low numbers of users (e.g. a few hundred Claim Technicians) - the biggest challenge is: how do you get the right information in front of them.
...
Let's imagine a correspondence collection process - it deals with unstructured data, where the data may not even have an explicit name. The goal how much can you learn to get a high level of customer service?

Classification (claim? submission? misc email box)

Continually make it more useful, "routing the event" (e.g. might send to lexusnexus for employee count)

Has this person ever filed a claim. Have we done business with them before?

The journey to policy administration.

(Like retail, inbound direct (messy), via broker, a REST service)

Correspondence happens at a point in time, give it a checksum, anything new links back to that hash/ID

Catalog when change happened, e.g. an underwriter overriding an industry code

The thing that's different to CRM is that there's brokers involved, and it's more about capacity management

<Fiddle>
  <Txs txs="INSERT INTO policies (_id, definition) VALUES (1, {foo: 'bar'});" />
  <Txs txs="UPDATE policies SET definition = {foo: 'baz'} WHERE _id = 1;" />
  <Query q={
`SELECT *
  FROM policies
`}
...
In common SQL databases, the original definition is lost forever, and no information about when it was last changed is recorded. Perhaps if you're sufficiently desparate and very lucky there _might_ be an old backup of the data available somewhere with a previous value.
...
`SELECT *, _system_from, _system_to
  FROM policies FOR ALL SYSTEM_TIME
`}
/>
</Fiddle>

Underwriter: we don't like the result, let's send previous data (pre-enrichment) to other third-party. Or send both! SIC code mutating from one row to the next.

Imagine the leverage and underwriter can get if they can traverse time in the queries: how do you correlate quality/time-effectiveness with the processes applied to flows.
We don't have good metrics because we don't have good grasp of unstructured data
"Have I seen anything like this lately?", submission by a competitor 180 days ago
...
Underwriters need to run calculations against the best known version of the correspondence. Often a configured blackbox is used. Needs reference data, e.g. public data sets, weather data, flora

Underwriters wants point-in-time reporting, let me think about what my best move should have been (observations at a point in time)

Keeping copies of URLs

After initial quoting, might cross-sell and up-sell.
...
M&A data, record business as of last year

Working on a system that's flawed, or multiple
Underwriter assistant might have to work across systems, underwriter doesn't want to have to care about which system
audit for performance improvement (could be commission best)
audit for learning what tools to build next
insurance companies are thinking about what's "discoverable" (might leverage right to be forgotten)
...
No schema, no pydantic, you get a pile of JSON, it's a mess
...
OCR mistake, wrong date

Duration of policy is manual negotiation

Renewals can sometimes use policy numbers
