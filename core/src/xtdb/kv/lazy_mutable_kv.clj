...
(defn- sync! [^xtdb.kv.KvStore kv offset ^ArrayList kvv]
  (let [size (.size kvv)]
    (kv/store kv (.subList kvv @offset size))
    (reset! offset size)))

(deftype LazyMutableKvSnapshot [^xtdb.kv.KvStore kv offset ^ArrayList kvv]
  kv/KvSnapshot
...
(deftype LazyKvStore [^xtdb.kv.KvStore kv offset ^ArrayList kvv]
  kv/KvStore
...
(defn ->lazy-mutable-kv-store []
  (->LazyKvStore (mutable-kv/->mutable-kv-store) (atom 0) (ArrayList.)))
